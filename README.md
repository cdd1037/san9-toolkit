...
- 启动器发现同目录 `font.ttf` 后，会在恢复游戏主线程前把字体路径交给 Runtime。
  Runtime 用 DirectWrite 直接加载字体文件，不安装系统字体。普通 Big5 中文、英文、数字和
  标点沿用原作逐字位置、12/16/24 像素字号、颜色、效果与裁剪，作为保留式文字命令随
  `KoeiSurface` 复制链传播，最后由 Direct2D 在物理分辨率绘制；私用字形和缺字仍由原作绘制。
  原作普通字形只在其本次绘制事务中生成以验证覆盖范围，随后立即恢复调用前的目标像素，
  不在 Present 阶段清字，也不保存或推断历史背景。后续位图合成、整面填充和直接图元写入
  按实际调用顺序裁剪其下方文字命令。
...
字体替换需要同目录存在名为 `font.ttf` 的单字体文件；缺失或无法加载时，其余功能正常启用，
文字保持原作效果。字体文件不包含在仓库和构建产物中。

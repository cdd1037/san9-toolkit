...
- 启动器发现同目录 `font.ttf` 后，会把其绝对路径交给 Runtime。Runtime 不安装字体，
  而是使用 DirectWrite 直接加载该文件，并在所有缩放倍率下替换原作普通中英文、数字和
  标点；布局、字号、颜色、描边、裁剪和前进量仍沿用原作记录。原作私用字形、字体缺字或
  字体加载失败时保留原字形。文字元数据随离屏表面复制链传播，Direct2D 在 Catmull–Rom
  缩放后以物理分辨率绘制最终文字层。
...
若要启用字体替换，同时在该目录放置名为 `font.ttf` 的单字体文件；字体文件不包含在仓库
或构建产物中。没有该文件时，其余缩放和输入功能照常工作。

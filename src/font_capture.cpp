...
constexpr std::uintptr_t kFillSurfaceRva = 0x1BEB00;
constexpr std::uintptr_t kFillHorizontalRva = 0x1C2810;
constexpr std::uintptr_t kPutPixelRva = 0x1C29D0;
constexpr std::uintptr_t kDrawHorizontalRva = 0x1C2AB0;
constexpr std::uintptr_t kDrawVerticalRva = 0x1C2CA0;
constexpr std::uintptr_t kFillRectangleRva = 0x1C3250;
constexpr std::uintptr_t kFillCheckerRva = 0x1C3310;
constexpr std::uintptr_t kDrawVerticalRgbRva = 0x1C33D0;
...
using FillSurfaceFunction = short(__thiscall*)(GameSurface*, int);
using FillHorizontalFunction = char*(__thiscall*)(GameSurface*, int, int, int, int);
using PutPixelFunction = char*(__thiscall*)(GameSurface*, int, int, int);
using DrawHorizontalFunction = char*(__thiscall*)(GameSurface*, int, int, int, unsigned int);
using DrawVerticalFunction = char*(__thiscall*)(GameSurface*, int, int, int, unsigned int);
using FillRectangleFunction = unsigned int(__thiscall*)(GameSurface*, int, int, int, int, int);
using FillCheckerFunction = unsigned int(__thiscall*)(GameSurface*, int, int, int, int, int);
using DrawVerticalRgbFunction = std::uint16_t*(__thiscall*)(GameSurface*, int, int, int, int);
...
struct SurfaceState {
    ...
    int stride{};
    bool topDown{};
    std::vector<std::uint8_t> transactionPixels;
...
FillSurfaceFunction g_originalFillSurface = nullptr;
FillHorizontalFunction g_originalFillHorizontal = nullptr;
PutPixelFunction g_originalPutPixel = nullptr;
DrawHorizontalFunction g_originalDrawHorizontal = nullptr;
DrawVerticalFunction g_originalDrawVertical = nullptr;
FillRectangleFunction g_originalFillRectangle = nullptr;
FillCheckerFunction g_originalFillChecker = nullptr;
DrawVerticalRgbFunction g_originalDrawVerticalRgb = nullptr;
...
std::vector<MaskRun> BuildMask(const SurfaceState& state, const RECT& rectangle) {
    ...
    for (int y = rectangle.top; y < rectangle.bottom; ++y) {
        ...
        const auto* clean = reinterpret_cast<const std::uint16_t*>(
            RowPointer(state, state.transactionPixels.data(), y));
        int start = -1;
...
std::vector<MaskRun> BuildOwnership(const RECT& rectangle) {
    std::vector<MaskRun> ownership;
    ownership.reserve(static_cast<std::size_t>(rectangle.bottom - rectangle.top));
    for (int y = rectangle.top; y < rectangle.bottom; ++y) {
        ownership.push_back(MaskRun{y, rectangle.left, rectangle.right});
    }
    return ownership;
}

void RestoreRuns(SurfaceState& state, const std::vector<MaskRun>& runs) {
    for (const MaskRun& run : runs) {
        const auto* source = reinterpret_cast<const std::uint16_t*>(
            RowPointer(state, state.transactionPixels.data(), run.y));
        auto* destination = reinterpret_cast<std::uint16_t*>(
            RowPointer(state, state.pixels, run.y));
        std::copy(source + run.left, source + run.right, destination + run.left);
    }
}
...
SurfaceState& ResetState(const GameSurface& surface) {
    ...
    state.transactionPixels.resize(byteCount);
    state.coverageProbe.resize(byteCount);
    std::memcpy(state.transactionPixels.data(), surface.pixels, byteCount);
    state.glyphs.clear();
    return state;
}
...
void CaptureTransactionPixels(SurfaceState& state, const RECT& rectangle) {
    for (int y = rectangle.top; y < rectangle.bottom; ++y) {
        const auto* source = RowPointer(state, state.pixels, y) + rectangle.left * 2;
        auto* destination = RowPointer(state, state.transactionPixels.data(), y) + rectangle.left * 2;
...
void __fastcall HookRenderFontGlyphs(void* layout, void*, int originX, int originY,
        ...
        ReadGlyphs(layout, originX, originY, *clip, *destination);
    {
        ...
        for (const PendingGlyph& glyph : pending) {
            // Capture the destination as it exists immediately before this
            // concrete font transaction. Existing retained text has no legacy
            // pixels in the surface, so these are the authoritative background
            // pixels even for animated content.
            CaptureTransactionPixels(state, glyph.pixels);
            if (glyph.replaceable) {
                ApplyCoverage(state, BuildOwnership(glyph.pixels));
            }
        }
    }
    ...
    for (const PendingGlyph& glyph : pending) {
        ...
        const std::vector<MaskRun> rasterizedPixels = BuildMask(state, glyph.pixels);
        if (!rasterizedPixels.empty()) {
            // Suppress the legacy bitmap glyph in the same draw transaction. The
            // saved pixels are the exact destination contents immediately before
            // this call, so no historical background reconstruction is involved.
            RestoreRuns(state, rasterizedPixels);
            ...
            state.glyphs.push_back(
                TrackedGlyph{glyph.draw, glyph.pixels, BuildOwnership(glyph.pixels), {}});
        }
    }
}
...
int __fastcall HookBlitSurface(GameSurface* source, void*, GameSurface* destination,
    ...
    for (const TrackedGlyph& sourceGlyph : sourceGlyphs) {
        ...
        draw.bounds = pixels;
        ApplyCoverage(destinationState, mask);
...
void OccludeRectangle(GameSurface* surface, RECT requested) {
    ...
    RECT clipped{};
    if (!Intersect(requested, SurfaceBounds(*surface), clipped)) {
        return;
    }
    ApplyCoverage(found->second, BuildOwnership(clipped));
}

short __fastcall HookFillSurface(GameSurface* surface, void*, int value) {
    const short result = g_originalFillSurface(surface, value);
    if (IsUsableSurface(surface)) {
        std::scoped_lock lock(g_mutex);
        const auto found = g_surfaces.find(surface->pixels);
        if (found != g_surfaces.end()) {
...
char* __fastcall HookFillHorizontal(GameSurface* surface, void*, int left, int right,
                                    int top, int color) {
    char* const result = g_originalFillHorizontal(surface, left, right, top, color);
    OccludeRectangle(surface, RECT{left, top, right + 1, top + 1});
    return result;
}

char* __fastcall HookPutPixel(GameSurface* surface, void*, int x, int y, int value) {
    char* const result = g_originalPutPixel(surface, x, y, value);
    OccludeRectangle(surface, RECT{x, y, x + 1, y + 1});
    return result;
}

char* __fastcall HookDrawHorizontal(GameSurface* surface, void*, int left, int right,
                                    int y, unsigned int color) {
    char* const result = g_originalDrawHorizontal(surface, left, right, y, color);
    OccludeRectangle(surface, RECT{std::min(left, right), y,
                                   std::max(left, right) + 1, y + 1});
    return result;
}

char* __fastcall HookDrawVertical(GameSurface* surface, void*, int top, int bottom,
                                  int x, unsigned int color) {
    char* const result = g_originalDrawVertical(surface, top, bottom, x, color);
    OccludeRectangle(surface, RECT{x, std::min(top, bottom), x + 1,
                                   std::max(top, bottom) + 1});
    return result;
}

unsigned int __fastcall HookFillRectangle(GameSurface* surface, void*, int x, int y,
                                          int width, int height, int color) {
    const unsigned int result = g_originalFillRectangle(surface, x, y, width, height, color);
    OccludeRectangle(surface, RECT{x, y, x + width, y + height});
    return result;
}

unsigned int __fastcall HookFillChecker(GameSurface* surface, void*, int x, int y,
                                        int width, int height, int color) {
    const unsigned int result = g_originalFillChecker(surface, x, y, width, height, color);
    OccludeRectangle(surface, RECT{x, y, x + width, y + height});
    return result;
}

std::uint16_t* __fastcall HookDrawVerticalRgb(GameSurface* surface, void*, int top,
                                             int bottom, int x, int color) {
    std::uint16_t* const result = g_originalDrawVerticalRgb(surface, top, bottom, x, color);
    OccludeRectangle(surface, RECT{x, std::min(top, bottom), x + 1,
                                   std::max(top, bottom) + 1});
    return result;
}
...
bool InstallHooks() {
    ...
    void* fillTrampoline = nullptr;
    void* fillHorizontalTrampoline = nullptr;
    void* putPixelTrampoline = nullptr;
    void* drawHorizontalTrampoline = nullptr;
    void* drawVerticalTrampoline = nullptr;
    void* fillRectangleTrampoline = nullptr;
    void* fillCheckerTrampoline = nullptr;
    void* drawVerticalRgbTrampoline = nullptr;
    ...
    constexpr std::array<unsigned char, 6> fillExpected{
        0x83, 0xEC, 0x08, 0x55, 0x8B, 0xE9};
    if (!code_hook::Install(module + kFillSurfaceRva,
                            reinterpret_cast<void*>(&HookFillSurface), fillExpected,
                            &fillTrampoline)) {
        return false;
    }
    g_originalFillSurface = reinterpret_cast<FillSurfaceFunction>(fillTrampoline);
    ...
    constexpr std::array<unsigned char, 7> fillHorizontalExpected{
        0x8B, 0x44, 0x24, 0x10, 0x8A, 0xD4};
    constexpr std::array<unsigned char, 7> putPixelExpected{
        0x8B, 0x44, 0x24, 0x08, 0x85, 0xC0};
    constexpr std::array<unsigned char, 7> drawHorizontalExpected{
        0x83, 0xEC, 0x18, 0x8B, 0x44, 0x24, 0x28};
    constexpr std::array<unsigned char, 7> drawVerticalExpected{
        0x83, 0xEC, 0x18, 0x8B, 0x44, 0x24, 0x28};
    constexpr std::array<unsigned char, 8> fillRectangleExpected{
        0x8B, 0x44, 0x24, 0x08, 0x55, 0x56, 0x8B, 0xF1};
    constexpr std::array<unsigned char, 7> fillCheckerExpected{
        0x53, 0x55, 0x8B, 0x6C, 0x24, 0x10, 0x56};
    constexpr std::array<unsigned char, 9> drawVerticalRgbExpected{
        0x8B, 0x44, 0x24, 0x10, 0x8A, 0xD4, 0x80, 0xE2, 0xF8};
    const bool primitivesInstalled =
        code_hook::Install(module + kFillHorizontalRva,
                           reinterpret_cast<void*>(&HookFillHorizontal), fillHorizontalExpected,
                           &fillHorizontalTrampoline) &&
        code_hook::Install(module + kPutPixelRva,
                           reinterpret_cast<void*>(&HookPutPixel), putPixelExpected,
                           &putPixelTrampoline) &&
        code_hook::Install(module + kDrawHorizontalRva,
                           reinterpret_cast<void*>(&HookDrawHorizontal), drawHorizontalExpected,
                           &drawHorizontalTrampoline) &&
        code_hook::Install(module + kDrawVerticalRva,
                           reinterpret_cast<void*>(&HookDrawVertical), drawVerticalExpected,
                           &drawVerticalTrampoline) &&
        code_hook::Install(module + kFillRectangleRva,
                           reinterpret_cast<void*>(&HookFillRectangle), fillRectangleExpected,
                           &fillRectangleTrampoline) &&
        code_hook::Install(module + kFillCheckerRva,
                           reinterpret_cast<void*>(&HookFillChecker), fillCheckerExpected,
                           &fillCheckerTrampoline) &&
        code_hook::Install(module + kDrawVerticalRgbRva,
                           reinterpret_cast<void*>(&HookDrawVerticalRgb), drawVerticalRgbExpected,
                           &drawVerticalRgbTrampoline);
    if (!primitivesInstalled) {
        return false;
    }
    g_originalFillHorizontal = reinterpret_cast<FillHorizontalFunction>(fillHorizontalTrampoline);
    g_originalPutPixel = reinterpret_cast<PutPixelFunction>(putPixelTrampoline);
    g_originalDrawHorizontal = reinterpret_cast<DrawHorizontalFunction>(drawHorizontalTrampoline);
    g_originalDrawVertical = reinterpret_cast<DrawVerticalFunction>(drawVerticalTrampoline);
    g_originalFillRectangle = reinterpret_cast<FillRectangleFunction>(fillRectangleTrampoline);
    g_originalFillChecker = reinterpret_cast<FillCheckerFunction>(fillCheckerTrampoline);
    g_originalDrawVerticalRgb = reinterpret_cast<DrawVerticalRgbFunction>(drawVerticalRgbTrampoline);
    g_enabled = true;
    return true;
}
